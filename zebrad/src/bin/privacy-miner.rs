...
Use the same network configuration as your node: a custom Testnet, or SwarmMainnet for SWARM\n\
production. Set [mining].miner_address\n\
on that node to your chosen payout first. This client verifies the coinbase recipient.\n\
One CPU worker; --blocks 0 (default) runs until Ctrl+C. Upstream Mainnet and Regtest are refused.\n\
...
It is not a mainnet launch genesis or final economic configuration.";
    ...
    pub async fn run() -> Result<()> {
        ...
        // SWARM production is accepted alongside a PoW Testnet: it is a SWARM-owned network with
        // its own difficulty limit and its own address prefixes, and refusing it here would mean
        // the chain the tool exists for could not be mined with it. Upstream Zcash Mainnet and
        // Regtest are still refused.
        let network = &config.network.network;
        let is_swarm_main = network.kind() == NetworkKind::SwarmMainnet;
        if (network.kind() != NetworkKind::Testnet && !is_swarm_main) || network.disable_pow() {
            bail!(
                "This miner requires a PoW-enabled Testnet or SwarmMainnet; \
                 upstream Mainnet and Regtest are refused"
            );
        }
        ...
        if payout.network_kind() != network.kind() {
