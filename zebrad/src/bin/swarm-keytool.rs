...
const HELP: &str = "swarm-keytool — offline SWARM testnet P2SH destination addresses\n\
\n\
Usage:\n\
...
OP_CHECKMULTISIG, and prints the testnet P2SH address (t2…), the redeem\n\
...
fn p2sh_testnet_address(script: &[u8]) -> Address {
    Address::from_script_hash(NetworkKind::Testnet, hash160(script))
}

// -- subcommands -------------------------------------------------------------

fn cmd_address(args: &[String]) -> Result<(), String> {
    let parsed = options(args, &["redeem-script"])?;
...
fn cmd_new(args: &[String]) -> Result<(), String> {
    let parsed = options(args, &["threshold", "keys", "label", "out"])?;
    ...
    write_key_file(&path, &label, threshold, &address, &script, &secrets, &pubkeys)?;
...
fn write_key_file(
    ...
    let document = serde_json::json!({
        ...
        "tool": "swarm-keytool",
        "purpose": "SWARM testnet funding-stream destination (P2SH multisig); not a mainnet key ceremony",
        "label": label,
        "network": "testnet",
